// CoursesScreen.js (replace entire file with this)
...
// <-- IMPORTANT: keep your app-wide styles import (you had many references to it) -->
import styles from '../styles/styles';

// --- Local camera-only styles (so we don't overwrite your imported styles) ---
const cameraStyles = StyleSheet.create({
  ...
  zoomBadgeText: { color: 'white', fontSize: 12 },
});

// --- Custom Alert/Error Modal Component ---
      ...
      <View style={styles.modalOverlay}>
        <View style={[styles.modalContainer, { backgroundColor: theme.colors.surface, height: 'auto' }]}>
          ...
          <ScrollView contentContainerStyle={{ paddingVertical: 10 }}>
            <TextInput label="Deadline Name (e.g. Quiz 1)" value={deadlineName} onChangeText={setDeadlineName} style={styles.textInput} mode="outlined" />
            ...
            {showDatePicker && (
              <DatePickerModal locale="en" mode="single" visible={showDatePicker} onDismiss={() => setShowDatePicker(false)} date={deadlineDate} onConfirm={onDateConfirm} />
            )}
            {showTimePicker && <DateTimePicker value={deadlineDate} mode="time" display="default" onChange={onTimeChange} />}
...
// --- GeneralDeadlineModal (same pattern) ---
    ...
    <Modal animationType="slide" transparent={true} visible={visible} onRequestClose={onClose}>
      <View style={styles.modalOverlay}>
        <View style={[styles.modalContainer, { backgroundColor: theme.colors.surface, height: 'auto' }]}>
          ...
          <ScrollView contentContainerStyle={{ paddingVertical: 10 }}>
            <TextInput label="Deadline Name (e.g. Internship Application)" value={deadlineName} onChangeText={setDeadlineName} style={styles.textInput} mode="outlined" />
...
// --- NotesModal (re-added so code compiles) ---
...
// --- CameraModal (named component, NOT default export) ---
function CameraModal({ visible, onClose, onPictureSaved }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // optional microphone permission if you plan to record video
  ...
  const takePicture = async () => {
    try {
      if (cameraRef.current && cameraRef.current.takePictureAsync) {
        ...
        console.warn('Camera ref is not ready or takePictureAsync not found.');
      }
    } catch (err) {
  ...
  return (
    <Modal visible={visible} animationType="slide">
      <View style={{ flex: 1, backgroundColor: 'black' }}>
        ...
        <View style={cameraStyles.zoomBadge}>
          <Text style={cameraStyles.zoomBadgeText}>Zoom: {(zoom * 100).toFixed(0)}%</Text>
        </View>

        <View style={cameraStyles.bottomBar}>
          <TouchableOpacity onPress={() => setZoom((z) => Math.max(0, +(z - 0.1).toFixed(2)))} style={cameraStyles.iconButton}>
          ...
          <TouchableOpacity onPress={() => setZoom((z) => Math.min(1, +(z + 0.1).toFixed(2)))} style={cameraStyles.iconButton}>
...
// --- CoursesScreen Component (default export) ---
const CoursesScreen = () => {
  const theme = useTheme();
  ...
  const handleAddDeadline = (courseId, deadlineData) => {
    setCourses((prev) => prev.map((course) => (course.id === courseId ? { ...course, deadlines: [...course.deadlines, deadlineData] } : course)));
  };

  const handlePictureSaved = (photoUri) => {
    if (!selectedCourse) return;
    setCourses((prev) => prev.map((course) => (course.id === selectedCourse.id ? { ...course, notes: [...course.notes, photoUri] } : course)));
  };

  const handleRemoveNote = (noteIndex) => {
    if (!selectedCourse) return;
    setCourses((prev) => prev.map((course) => (course.id === selectedCourse.id ? { ...course, notes: course.notes.filter((_, idx) => idx !== noteIndex) } : course)));
  };
        ...
        {item.deadlines.length > 0 ? (
          item.deadlines.map((deadline) => (
            <View key={deadline.id} style={{ marginBottom: 10 }}>
              ...
              <Paragraph style={{ color: calculateTimeLeft(deadline.date) === 'Passed' ? theme.colors.error : theme.colors.primary }}>{calculateTimeLeft(deadline.date)}</Paragraph>
            </View>
          ))
        ) : (
  ...
  return (
    <View style={[styles.screenContainer, { backgroundColor: theme.colors.background }]}>
      ...
      {/* Modals */}
      <DeadlineModal visible={isDeadlineModalVisible} onClose={() => setIsDeadlineModalVisible(false)} courseId={selectedCourse?.id} onAddDeadline={handleAddDeadline} />
      <CameraModal visible={isCameraVisible} onClose={() => setIsCameraVisible(false)} onPictureSaved={handlePictureSaved} />
      <NotesModal visible={isNotesModalVisible} onClose={() => setIsNotesModalVisible(false)} notes={selectedCourse?.notes || []} onRemoveNote={handleRemoveNote} />
      <GeneralDeadlineModal visible={isGeneralDeadlineModalVisible} onClose={() => setIsGeneralDeadlineModalVisible(false)} onAddDeadline={handleAddGeneralDeadline} />
    </View>
  );
};
